...
(function () {
  ...
  function makeHdrIcon(id, svgHtml, titleText) {
    ...
    btn.id = id;
    btn.title = titleText;
    // Белый квадрат-подложка, обведённый в стиль коробки-кота: тонкий тёмный контур #5F6368 1.5px, без тени.
    // Обводим саму подложку, а не глиф — ключ и магнит остаются сплошными.
  ...
  /* ---------- уведомление «поддержка закончилась» (Добби) ---------- */
  ...
  // Оповещение = Добби: выезжает справа, фон слегка голубеет, вокруг салют. Щелчок / Esc — закрыть.
  function closeSupportAlert(animate) {
    const box = document.getElementById('eduson-support-alert'), tint = document.getElementById('eduson-support-tint');
    if (!box) return;
    const kill = function () { box.remove(); if (tint) tint.remove(); };
    if (!animate) { kill(); return; }
    box.classList.add('out');
    if (tint) tint.classList.add('out');
    setTimeout(kill, 450);
  }
  // Салют: 7 ракет подряд в правой части экрана, у каждой свой цвет и место.
  function dobbyFireworks() {
    const layer = document.createElement('div'), cols = ['#60a5fa', '#facc15', '#f472b6', '#ffffff', '#34d399'];
    layer.className = 'dob-fw';
    for (let k = 0; k < 7; k++) {
      const b = document.createElement('div'), r = 60 + Math.random() * 40;
      b.className = 'dob-b';
      b.style.cssText = 'left:' + (35 + Math.random() * 60) + '%;top:' + (10 + Math.random() * 40) + '%;--c:' + cols[k % 5] + ';--t:' + (0.8 + k * 0.45).toFixed(2) + 's';
      b.innerHTML = '<span class="r"></span>' + Array.from({ length: 16 }, function (_, i) {
        const a = i / 16 * Math.PI * 2;
        return '<span class="p" style="--x:' + Math.round(Math.cos(a) * r) + 'px;--y:' + Math.round(Math.sin(a) * r) + 'px"></span>';
      }).join('');
      layer.appendChild(b);
    }
    return layer;
  ...
  function showSupportAlert(supDate, reqDate) {
    ...
    if (!document.getElementById('eduson-support-alert-css')) {
      ...
      st.textContent =
        '#eduson-support-tint{position:fixed;inset:0;z-index:2147483646;pointer-events:none;background:rgba(96,165,250,.16);animation:dobFade .6s}' +
        '#eduson-support-tint.out{animation:dobFade .4s reverse forwards}' +
        '@keyframes dobFade{from{opacity:0}}' +
        '#eduson-support-alert{position:fixed;right:0;bottom:0;z-index:2147483647;display:flex;align-items:flex-end;cursor:pointer;font-family:' + HP_FONT + ';animation:dobIn .7s cubic-bezier(.2,1.25,.4,1)}' +
        '#eduson-support-alert.out{animation:dobOut .45s ease-in forwards}' +
        '@keyframes dobIn{from{transform:translateX(115%)}}' +
        '@keyframes dobOut{to{transform:translateX(115%)}}' +
        '#eduson-support-alert img{height:280px;transform-origin:50% 100%;animation:dobSway 3.4s ease-in-out 1s infinite}' +
        '@keyframes dobSway{0%,100%{transform:rotate(1.2deg)}50%{transform:rotate(-1.6deg)}}' +
        '.dob-l{display:flex;flex-direction:column;align-items:flex-end;gap:14px;margin:0 18px 34px 0}' +
        // облако: «горбы» сверху и снизу (кружки и их тени) + общая обводка через drop-shadow
        '.dob-cl{position:relative;isolation:isolate;max-width:240px;padding:20px 26px;background:#fff;border-radius:36px;text-align:center;font-size:19px;line-height:1.3;font-weight:800;color:#1e3a8a;' +
          'transform-origin:100% 100%;filter:drop-shadow(2px 0 0 #93c5fd) drop-shadow(-2px 0 0 #93c5fd) drop-shadow(0 2px 0 #93c5fd) drop-shadow(0 -2px 0 #93c5fd) drop-shadow(0 10px 14px rgba(30,58,138,.22));' +
          'animation:dobPop .65s .5s cubic-bezier(.3,1.7,.5,1) backwards,dobFloat 3.4s 1.2s ease-in-out infinite}' +
        '.dob-cl:before,.dob-cl:after{content:"";position:absolute;z-index:-1;background:#fff;border-radius:50%}' +
        '.dob-cl:before{width:74px;height:74px;top:-30px;left:20px;box-shadow:96px 10px 0 -8px #fff}' +
        '.dob-cl:after{width:60px;height:60px;bottom:-24px;left:46px;box-shadow:94px -6px 0 -6px #fff}' +
        '.dob-cl .t{position:absolute;background:#fff;border-radius:50%;right:-14px;bottom:2px;width:18px;height:18px;animation:dobDot 1.6s ease-in-out infinite}' +
        '.dob-cl .t+.t{right:-30px;bottom:-16px;width:11px;height:11px;animation-delay:.3s}' +
        // слова в облаке проявляются по одному
        '.dob-cl .w{display:inline-block;margin-right:.28em;opacity:0;animation:dobW .35s calc(1.1s + var(--i)*.16s) forwards}' +
        '@keyframes dobW{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}' +
        '@keyframes dobPop{from{transform:scale(0);opacity:0}}' +
        '@keyframes dobFloat{0%,100%{translate:0 0}50%{translate:0 -7px}}' +
        '@keyframes dobDot{0%,100%{scale:1}50%{scale:1.35}}' +
        '.dob-pill{background:#fff;color:#1e3a8a;border:2px solid #93c5fd;border-radius:999px;padding:6px 14px;font-size:13px;font-weight:700;box-shadow:0 4px 10px rgba(30,58,138,.15);animation:dobFade .5s 1s backwards}' +
        // салют: ракета взлетает и рассыпается искрами
        '.dob-fw{position:absolute;inset:0;overflow:hidden}' +
        '.dob-b{position:absolute;width:0;height:0}' +
        '.dob-b .r{position:absolute;left:-2px;width:4px;height:28px;border-radius:4px;opacity:0;background:linear-gradient(transparent,var(--c));animation:dobUp .6s var(--t) ease-out forwards}' +
        '.dob-b .p{position:absolute;left:-3px;top:-3px;width:6px;height:6px;border-radius:50%;opacity:0;background:var(--c);box-shadow:0 0 8px var(--c);animation:dobBoom 1.2s calc(var(--t) + .55s) ease-out forwards}' +
        '@keyframes dobUp{0%{opacity:1;transform:translateY(60vh)}90%{opacity:1}100%{opacity:0;transform:none}}' +
        '@keyframes dobBoom{0%{opacity:1;transform:none}65%{opacity:1}100%{opacity:0;transform:translate(var(--x),calc(var(--y) + 34px)) scale(.3)}}';
      document.head.appendChild(st);
      document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && document.getElementById('eduson-support-alert')) closeSupportAlert(true);
      }, true);
    }
    const days = Math.max(1, Math.round((reqDate - supDate) / 86400000));
    const words = 'У Добби больше нет хозяина. Добби — свободный эльф'.split(' ').map(function (w, i) {
      return '<span class="w" style="--i:' + i + '">' + w + '</span>';
    }).join('');
    const tint = document.createElement('div');
    tint.id = 'eduson-support-tint';
    tint.appendChild(dobbyFireworks());
    const box = document.createElement('div');
    box.id = 'eduson-support-alert';
    box.title = 'Щёлкните, чтобы закрыть';
    box.innerHTML = '<div class="dob-l"><div class="dob-cl">' + words + '<span class="t"></span><span class="t"></span></div>' +
      '<div class="dob-pill">Поддержка была до ' + fmtDMY(supDate) + ' · обращение ' + fmtDMY(reqDate) + ' (позже на ' + days + ' дн.)</div></div>' +
      '<img alt="" src="' + DOBBY_IMG + '">';
    box.onclick = function () { closeSupportAlert(true); };
    document.documentElement.append(tint, box);
  }
  // Добби: обрезан по контуру, ужат до 252×280, WebP (~17 КБ вместо 1,5 МБ в исходном SVG).
  const DOBBY_IMG = 'data:image/webp;base64,';
  ...
  function buildBoard() {
    ...
    function viewFree(inner) {
      ...
      const ttl = elt('div', SERIF + 'font-size:14.5px;line-height:1.45;white-space:nowrap;overflow:hidden;', 'Здесь еще никто не мурчал');
      ttl.appendChild(mkCat());
  ...
  // цен часто не несёт («Excel и Google-таблицы» на деле — тариф Мастер курса «Excel», Наталья
  // поймала на живом поиске). При нескольких совпадениях по цене уточняем по общим словам названия.
  function giftMatchByPrice(c, giftList) {
    const target = Math.round(+c.price_from);
    if (!target) return null;
    const cands = giftList.filter(function (o) { return Math.round(+o.price) === target; });
    if (!cands.length) return null;
    if (cands.length === 1) return cands[0];
    const stems = pcNorm(c.course_name).split(' ').filter(function (w) { return w.length > 2; });
    let best = null, bestScore = -1;
    cands.forEach(function (o) {
      const n = pcNorm(o.course);
      const score = stems.filter(function (w) { return n.indexOf(w) !== -1; }).length;
      if (score > bestScore) { bestScore = score; best = o; }
    });
    return bestScore > 0 ? best : cands[0];
  }
  // Внутри одной ссылки каталога цен несколько тарифов делят одно название (см. «siblings» в
  // showResult) — размечаем их коротким «база + тариф» (напр. «Excel Мастер») по цене из
  // подарочного каталога. Группы из одного тарифа не трогаем — переименовывать нечего.
  function buildTariffLabels(catalog, giftList) {
    const byUrl = {};
    ...
    Object.keys(byUrl).forEach(function (key) {
      ...
      group.forEach(function (c) {
        ...
        const t = m && giftTariffLabel(m.course);
        if (t) labels.set(c, base + ' ' + t);
      });
    });
    return labels;
  }
  ...
  function renderPickCourse(body) {
    ...
    function showResult() {
      ...
      card.appendChild(elt('div', 'font-weight:900;font-size:15px;color:' + ACC_DEEP + ';margin-top:2px;', labelFor(c)));
    ...
    function studentMessage(info, groups) {
      ...
      groups.forEach(function (g) {
        lines.push(g.dir);
        g.items.forEach(function (c) { lines.push('• ' + labelFor(c) + (c.product_url ? ' - ' + c.product_url : '')); });
        lines.push('');
      });
      ...
      const drawList = function () {
        ...
        groups.forEach(function (g) {
          ...
          shown.forEach(function (c) {
            ...
            row.appendChild(elt('span', 'min-width:0;', labelFor(c)));
    ...
    // Как только оба каталога готовы — размечаем тарифы (buildTariffLabels) и перерисовываем
    ...
    // Не критично, если не загрузится — без него просто не будет коротких «база + тариф» подписей
    // (labelFor вернёт исходное имя из каталога цен, как раньше).
    loadGiftCatalog().then(function (list) { giftList = list; tryBuildLabels(); }).catch(function () {});
  ...
  /* ---------- под-вкладка «Новый аккаунт» (акция 1+1: подарочный курс другому человеку) ----------
     Клиент дарит курс другому и присылает его ФИО / почту / телефон. Заводим пользователя платформы
     как «Add new user» в админке (www.eduson.tv/admin/users/new): ФИО, почта, телефон, пароль 123456,
     язык «Русский». ⚠️ Рецепт проверен вручную на тестах (899469): если сразу поставить нужный курс,
     у аккаунта НЕТ группы Default (= «вне назначения») и вход даёт «слишком много перенаправлений».
     Группу Default НОВОГО курса платформа выдаёт при СМЕНЕ Company через «Edit user». Поэтому:
     1) создаём с промежуточным курсом, 2) меняем Company на нужный, 3) проверяем, что аккаунт появился
     в списке курса (/admin/companies/<id>/users — там только те, кто уже в группе).
     Суперюзер не нужен (вход у тестового аккаунта без него работает). */
  const NEWACC_PASSWORD = '123456';
  const NEWACC_HOLDERS = ['17538', '16021']; // промежуточные курсы (Операционный директор / 1С: Бухгалтерия 8.3) — берём тот, что не равен целевому
  ...
  function adminFormParams(html, mustHaveField) {
    ...
    return { params: params, action: form.getAttribute('action') || '' };
  }
  ...
  async function adminCreateUser(d) {
    ...
    if (looksLikeAdminLogin(html)) throw new Error('NOAUTH');
    const f = adminFormParams(html, 'user[email]');
  ...
  // «Edit user»: сменить Company (пароль и остальное — как в форме, без изменений)
  async function adminSetUserCompany(uid, companyId) {
    const html = await gmTextSlow(EDU_ADMIN + '/admin/users/' + uid + '/edit?language=ru');
    if (looksLikeAdminLogin(html)) throw new Error('NOAUTH');
    const f = adminFormParams(html, 'user[email]');
    f.params['user[company_id]'] = String(companyId);
    const action = f.action.indexOf('http') === 0 ? f.action : (EDU_ADMIN + (f.action || ('/admin/users/' + uid + '?language=ru')));
  ...
  async function adminUserInCompanyGroup(companyId, email, uid) {
    const html = await gmText(EDU_ADMIN + '/admin/companies/' + companyId + '/users?language=ru&q=' + encodeURIComponent(email));
    if (looksLikeAdminLogin(html)) throw new Error('NOAUTH');
    const doc = new DOMParser().parseFromString(html, 'text/html');
  ...
  function renderNewAccount(body) {
    ...
    body.appendChild(elt('div', 'font-size:10.5px;color:#64748B;font-weight:700;line-height:1.4;',
      'Регистрирует нового человека для подарочного курса: создаёт аккаунт и ставит курс так, чтобы платформа выдала ему группу (иначе вход не открывается).'));
      ...
      try {
        if (!userId) {
          ...
          if (!window.confirm('Создать аккаунт?\n\n' + fio + '\nПочта: ' + d.email + '\nТелефон: ' + (d.phone || '—') + '\nПароль: ' + d.password
            + '\nКурс: ' + picked.name + ' (Company ID ' + picked.id + ')\n\nСоздам аккаунт, затем поставлю нужный курс — так платформа выдаст группу.')) { busy = false; btn.style.opacity = '1'; btn.textContent = idleText; return; }

          btn.textContent = 'Создаю…'; status.textContent = 'Создаю аккаунт…';
          const holder = NEWACC_HOLDERS.find(function (h) { return h !== String(picked.id); });
          const res = await adminCreateUser({ last: d.last, first: d.first, middle: d.middle, email: d.email, phone: d.phone, password: d.password, companyId: holder });
          if (!res.id) {
            status.style.color = '#B45309';
        ...
        if (!moved) {
          status.textContent = 'Ставлю нужный курс…';
        ...
        console.log('[eduson-helper] новый аккаунт:', { userId: userId, company: picked.id, inGroup: inGroup });
        if (inGroup) {
          status.style.color = '#16A34A';
          status.textContent = '😻 Готово: аккаунт #' + userId + ', курс «' + picked.name + '», группа Default выдана.';
        } else {
          status.style.color = '#B45309';
          status.textContent = inGroup === false
            ? ('🙀 Аккаунт #' + userId + ' создан и курс поставлен, но в списке курса его нет — значит, группа не выдалась. Не отдавай клиенту ссылку, открой карточку и проверь «Groups».')
            : ('Аккаунт #' + userId + ' создан, курс «' + picked.name + '» поставлен. Проверить группу не получилось — открой карточку и убедись, что в «Groups» стоит Default.');
        }
